...
def _decrypt_feed_token(feed) -> str | None:
    ...
    for feed in feeds:
        try:
            if feed.feed_type == "cisa_kev":
                ...
                logger.warning("No ingest handler for feed type: %s", feed.feed_type)

            async with AsyncSessionLocal() as db:
                result = await db.execute(select(Feed).where(Feed.id == feed.id))
                f = result.scalar_one()
